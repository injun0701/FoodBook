...
//                self.itemAdd(page: 1, count: 10)
    ...
    func scrollItemAdd(page: Int, itemList: [Item], success: @escaping ([Item]) -> ()) {
        ...
        //서버에서 아이템 데이터 받아오기
        req.apiItemGet(page: page, count: 10, searchKeyWord: nil) { allcount, searchcount, list in
            print(list)
    ...
    //좋아요 삭제 메소드
    func itemLikeDelete(itemid: String, success: @escaping () -> ()) {
        //서버 통신을 위한 객체
        let req = URLRequest()
    ...
    //좋아요 업로드 메소드
    func itemLikeInsert(itemid: String, success: @escaping () -> ()) {
        //서버 통신을 위한 객체
        let req = URLRequest()
    ...
    //좋아요 메소드
    func itemLike(success: @escaping () -> ()) {
        //SQLite 파일 urlpath
        let directoryPath = SQLiteDocumentDirectoryPath()
        //마지막 업데이트 함수 파라미터 이름
