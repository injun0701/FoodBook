...
class SearchViewController: UIViewController {
    ...
    //검색어로 데이터 세팅
    func searchDataSetting() {
        let searchKeyWord = tfSearch.text ?? ""
        
        if tfSearch.text != "" {
            //데이터 세팅
            dataSetting(searchKeyWord: searchKeyWord)
        }
    }
    
    //데이터 세팅
    func dataSetting(searchKeyWord: String) {
        //서버에서 아이템 데이터 받아오기
        req.apiItemGet(page: 1, count: 10, searchKeyWord: searchKeyWord) { [self] allcount, searchcount, list in
            print(list)
    ...
    func searchScrollItemAdd(searchKeyWord: String, success: @escaping ([Item]) -> ()) {
        ...
        if flag == false && indexPath.row == self.itemList.count - 1 {
            ...
            if searchItemCountAll > self.itemList.count {
                ...
                networkCheck() { [self] in
                    ...
                    if tfSearch.text != "" {
                        searchScrollItemAdd(searchKeyWord: searchKeyWord) { itemList in
                            self.itemList = itemList
                            self.tableView.reloadData()
                        }
                        flag = false
